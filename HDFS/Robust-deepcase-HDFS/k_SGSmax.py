...
parser.add_argument('--a', default='536750', type=int, help='start of training')
parser.add_argument('--b', default='545000', type=int, help='end of trainign')
parser.add_argument('--time', default=60, type=int, help='time limit')
parser.add_argument('--rand_k', default=5, type=int, help='randomly chose k features to iterate')
...
class Attacker(object):
    ...
    def input_handle(self, funccall, y):  # 
        funccall = torch.tensor(funccall, dtype=float)
        t_diagnosis_codes = F.one_hot(funccall.to(torch.int64), self.n_labels).to(torch.float)
        t_diagnosis_codes = torch.unsqueeze(t_diagnosis_codes, dim=1)
        return t_diagnosis_codes.cuda()
