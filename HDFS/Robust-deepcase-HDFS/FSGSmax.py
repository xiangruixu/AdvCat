...
parser.add_argument('--Dtype', default='abnormal', type=str, help='test_dataset_type')
parser.add_argument('--D_k', default='1', type=int, help='grouped by k times for training')
parser.add_argument('--a', default='0', type=int, help='the start of training')
parser.add_argument('--b', default='2', type=int, help='the end of training')
...
class Attacker(object):
    ...
    def attack(self, funccall, y):
        ...
        while success_flag == 1:
            if len(greedy_set_visit_idx) == len(funccall):
                break
            iteration += 1
            success_flag = 1
            candidate_objects = []
            ...
            idx = np.argmax(candidate_objects)
            max_object = np.max(candidate_objects)
            max_pos = candidate_poses[idx]
            max_funccall = candidate_funccalls[idx]
        ...
        # print('flip_funccall:', flip_funccall)
...
for i in range(len(X)):
    ...
    print("* Original sample: " + str(sample), file=log_attack, flush=True)
